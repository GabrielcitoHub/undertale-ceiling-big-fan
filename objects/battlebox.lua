return function(x, y, w, h, tw, th) local self = {}
	...
	function self:makesoul(souls)
		...
		self.soul = self.souls[1] or Soul(xpos, ypos)
	...
	function self:update()
		...
		if not self.souls then self.souls = {self.soul} end
		for i, soul in ipairs(self.souls) do
			if soul and self.enabled then
				if soul.x < self.x + 13 then
					soul.x = self.x + 13
				end
				
				if soul.y < self.y + 13 then
					soul.y = self.y + 13
				end

				if soul.x > self.x + self.width - 13 then
					soul.x = self.x + self.width - 13
				end

				if soul.y > self.y + self.height - 13 then
					soul.y = self.y + self.height - 13
				end
			end
		end
	end
